...
const defaultSteps: StepDataInterface[] = [
  // LEARN MODE
  {
    ...
    description:
      "A plane mirror is a flat, smooth surface that reflects light. When you look into a mirror, you see your reflection!",
    visual: "mirror_intro",
  },
  {
    id: 2,
    ...
    description:
      "When light from an object hits the mirror, it reflects back. The reflected rays appear to come from behind the mirror, forming a virtual image.",
  ...
  },
  {
    ...
    description:
      "The image in a plane mirror is: 1) Same size as object, 2) Same distance behind mirror, 3) Laterally inverted, 4) Virtual and erect.",
    visual: "properties",
  },
  {
    id: 4,
    ...
    description:
      'Left appears as right and right appears as left. This is why "AMBULANCE" is written reversed on vehicles!',
  ...
  // PRACTICE MODE
  {
    ...
    description:
      "If an object is 5 cm in front of a mirror, where will the image be?",
    visual: "practice_position",
    interactionType: "position_quiz",
    correctAnswer: "5 cm behind the mirror",
    feedback: {
      correct:
        "Excellent! The image is always at the same distance behind the mirror as the object is in front.",
  ...
  },
  {
    ...
    title: "Image Size",
    description:
      "If you stand 2 meters from a mirror, how tall is your image compared to you?",
    visual: "practice_size",
    interactionType: "size_quiz",
    correctAnswer: "same",
    feedback: {
      correct:
        "Perfect! A plane mirror always forms an image of the same size.",
  ...
  },
  {
    ...
    description:
      "If you raise your right hand, which hand appears to be raised in the mirror?",
    visual: "practice_lateral",
    interactionType: "lateral_quiz",
    correctAnswer: "left",
    feedback: {
      correct:
        "Correct! The mirror reverses left and right - this is lateral inversion.",
      incorrect:
        "Remember: left becomes right and right becomes left in a mirror!",
    },
  },
  ...
  // REAL WORLD MODE
  {
    ...
    description:
      'Why is "AMBULANCE" written in reverse on emergency vehicles? So drivers can read it correctly in their rear-view mirrors!',
    visual: "ambulance",
  },
  {
    id: 9,
    ...
    description:
      "Barbers use mirrors to show you the back of your head. Two parallel mirrors create multiple reflections!",
    visual: "barber",
  },
  {
    id: 10,
    ...
    title: "Periscope",
    description:
      "Submarines use periscopes with two plane mirrors to see above water while staying submerged.",
    visual: "periscope",
  },
  ...
  // HANDS-ON MODE
  {
    ...
    description:
      "Move the object around and observe how the image changes! Notice the properties of reflection.",
    visual: "interactive",
  ...
  },
  {
    ...
    title: "Ray Tracing",
    description:
      "Draw light rays from the object to the mirror and see how they reflect to form the image.",
    visual: "ray_tracing",
...
const PlaneMirrorTool: React.FC<PlaneMirrorToolProps> = ({
  ...
  const propsWithDefaults = (props || {}) as NonNullable<
    PlaneMirrorToolProps["props"]
  >;
  const {
    width = 800,
    height = 600,
  ...
  const [drawnRays, setDrawnRays] = useState<
  ...
  useEffect(() => {
      ...
      rayPoints.forEach((point, index) => {
        ...
        if (progress > delay) {
          ...
          if (adjustedProgress > 0.5) {
            ...
            ctx.strokeStyle = `rgba(74, 77, 201, ${0.4 + index * 0.1})`;
            ctx.setLineDash([3, 3]);
            ctx.beginPath();
    ...
    const drawMeasurements = (objX: number, imgX: number, y: number) => {
      ...
      ctx.fillStyle = themeColor;
      ctx.fillText(
        `${Math.abs(objX - mirrorX).toFixed(0)}px`,
        (objX + mirrorX) / 2,
        y + objectHeight / 2 + 35,
      );
      ctx.fillStyle = "#4A4DC9";
      ctx.fillText(
        `${Math.abs(imgX - mirrorX).toFixed(0)}px`,
        (mirrorX + imgX) / 2,
        y + objectHeight / 2 + 45,
      );
    };
    ...
    if (currentStep.visual === "mirror_intro") {
      ...
      ctx.fillStyle = lightFlash ? "#4A4DC9" : "#dc2626";
      ...
      if (animationProgress > 0.65) {
        ...
        if (rayProg > 0) {
          ...
          const endX1 =
            ambulanceX + (mirrorX - 50 - ambulanceX) * Math.min(rayProg, 1);
          const endY1 =
            ambulanceY + (mirrorY + 15 - ambulanceY) * Math.min(rayProg, 1);
        ...
        if (rayProg > 0.6) {
          ctx.strokeStyle = "rgba(74, 77, 201, 0.8)";
          const prog2 = (rayProg - 0.6) * 2;
          ctx.beginPath();
          ctx.moveTo(mirrorX + 50, mirrorY + 15);
          const endX2 =
            mirrorX + 50 + (carX - mirrorX - 50) * Math.min(prog2, 1);
          const endY2 =
            mirrorY + 15 + (carY - 25 - mirrorY - 15) * Math.min(prog2, 1);
      ...
      // Mirror surface
      const mirror1Gradient = ctx.createLinearGradient(
        mirror1X - 75,
        0,
        mirror1X + 75,
        0,
      );
      mirror1Gradient.addColorStop(0, "#EBEBEB");
      mirror1Gradient.addColorStop(0.3, "#F5F5F5");
      mirror1Gradient.addColorStop(0.7, "#F5F5F5");
      mirror1Gradient.addColorStop(1, "#EBEBEB");
      ...
      if (animationProgress > 0.2) {
        ...
        const mirror2Gradient = ctx.createRadialGradient(
          mirror2X,
          mirror2Y,
          0,
          mirror2X,
      ...
      if (animationProgress > 0.6) {
        ...
        if (rayProgress > 0) {
          ...
          const endX =
            personX + 25 + (mirror1X - personX - 25) * Math.min(rayProgress, 1);
        ...
        if (rayProgress > 1) {
          ctx.strokeStyle = "rgba(74, 77, 201, 0.4)";
          ...
          const endX2 =
            mirror1X +
            75 +
            (mirror2X - 50 - mirror1X - 75) * Math.min(progress2, 1);
        ...
        if (rayProgress > 2) {
          ...
          const endX3 =
            mirror2X + 50 + (viewerX - mirror2X - 50) * Math.min(progress3, 1);
          const endY3 =
            mirror2Y - 20 + (viewerY - mirror2Y + 20) * Math.min(progress3, 1);
      ...
      if (animationProgress > 0.8) {
        ...
        // Label 1
        ctx.fillStyle = "rgba(74, 77, 201, 0.9)";
      ...
      for (let i = 0; i < 8; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          w * 0.85 + Math.cos(angle) * 30,
          h * 0.1 + Math.sin(angle) * 30,
        );
        ctx.lineTo(
          w * 0.85 + Math.cos(angle) * 45,
          h * 0.1 + Math.sin(angle) * 45,
        );
        ctx.stroke();
      }
      ...
      if (showRays && animationProgress > 0.4) {
        ...
        if (rayProgress > 1) {
          ...
          const endY2 =
            h * 0.3 - 20 + (subY - 50 - h * 0.3 + 20) * Math.min(progress2, 1);
        ...
        if (rayProgress > 2) {
          ctx.strokeStyle = "rgba(74, 77, 201, 0.7)";
          const progress3 = rayProgress - 2;
          ctx.beginPath();
      ...
      ctx.textAlign = "center";
      ctx.fillText(
        "Periscope: Two Plane Mirrors Help Submarines See Above Water!",
        w / 2,
        h * 0.97,
      );
    } else if (
      currentStep.visual === "practice_position" ||
      ...
      if (!isDragging) {
        ctx.strokeStyle = "rgba(74, 77, 201, 0.3)";
        ctx.lineWidth = 2;
        ctx.setLineDash([5, 5]);
        ctx.beginPath();
  ...
  const containerStyle: React.CSSProperties = {
    ...
    borderRadius: "16px",
    padding: "24px",
    boxShadow: "0 4px 24px rgba(74, 77, 201, 0.15)",
    display: "flex",
  ...
  const modeSelectorStyle: React.CSSProperties = {
    ...
    gap: "8px",
    padding: "4px",
    backgroundColor: darkMode ? "#2a2a2a" : "#C1C1EA",
    borderRadius: "12px",
  ...
  const getModeButton = (mode: ModeType, label: string, icon: string) => {
    ...
    const buttonStyle: React.CSSProperties = {
      ...
      color: isActive ? "#fff" : darkMode ? "#bbb" : "#4E4E4E",
      ...
      boxShadow: isActive ? "0 4px 12px rgba(74, 77, 201, 0.3)" : "none",
    };
  ...
  const renderPracticeQuiz = () => {
    ...
    const quizContainerStyle: React.CSSProperties = {
      ...
      borderRadius: "16px",
      padding: "32px",
      boxShadow: "0 20px 60px rgba(74, 77, 201, 0.2)",
      animation: "fadeInScale 0.5s ease",
      zIndex: 100,
      border: `2px solid ${darkMode ? "#374151" : "#EBEBEB"}`,
    };

    const questionStyle: React.CSSProperties = {
      marginBottom: "28px",
      paddingBottom: "24px",
      borderBottom: `2px solid ${darkMode ? "#374151" : "#EBEBEB"}`,
      textAlign: "center",
    };
    ...
    const getAnswerButton = (answer: string, label: string, index: number) => {
      ...
      const buttonStyle: React.CSSProperties = {
        padding: "16px 20px",
        border: showResult
          ? `3px solid ${isCorrect ? "#10b981" : "#ef4444"}`
          : showCorrectAnswer && showFeedback
            ? `3px solid #10b981`
            : `2px solid ${darkMode ? "#4b5563" : "#CACACA"}`,
        borderRadius: "12px",
        fontSize: "16px",
        fontWeight: "600",
        ...
        opacity: showFeedback && !isSelected && !showCorrectAnswer ? 0.4 : 1,
        position: "relative",
        textAlign: "left",
        display: "flex",
        alignItems: "center",
      ...
      return (
        ...
        >
          ...
          {showResult && <span style={iconStyle}>{isCorrect ? "✓" : "✗"}</span>}
          {showCorrectAnswer && !isSelected && <span style={iconStyle}>✓</span>}
        </button>
      );
    };
    ...
    return (
      <div style={quizContainerStyle}>
        <div style={questionStyle}>
          ...
          <p style={questionTextStyle}>{currentStep.description}</p>
        </div>
            ...
            style={{
              ...
              backgroundColor:
                selectedAnswer === currentStep.correctAnswer
                  ? "rgba(16, 185, 129, 0.15)"
                  : "rgba(239, 68, 68, 0.15)",
              border: `2px solid ${selectedAnswer === currentStep.correctAnswer ? "#10b981" : "#ef4444"}`,
              color:
                selectedAnswer === currentStep.correctAnswer
                  ? "#10b981"
                  : "#ef4444",
    ...
    const buttonStyle: React.CSSProperties = {
      ...
      backgroundColor: disabled
        ? darkMode
          ...
          : "#EBEBEB"
        : themeColor,
      color: disabled ? (darkMode ? "#666" : "#999") : "#fff",
      transition: "all 0.3s ease",
      display: "flex",
      alignItems: "center",
      gap: "8px",
      ...
      transform: "scale(1)",
      boxShadow: disabled ? "none" : "0 2px 8px rgba(74, 77, 201, 0.3)",
    };
  ...
  const progressBarStyle: React.CSSProperties = {
    flex: 1,
    height: "6px",
    backgroundColor: darkMode ? "#3a3a3a" : "#EBEBEB",
    borderRadius: "3px",
    overflow: "hidden",
    position: "relative",
  };
  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        ...
        {currentStep && <p style={descriptionStyle}>{currentStep.title}</p>}
      </div>

      {showModeSelector && (
        <div style={modeSelectorStyle}>
          {enabledModes.includes("learn") &&
            getModeButton("learn", "Learn", "📚")}
          {enabledModes.includes("practice") &&
            getModeButton("practice", "Practice", "✏️")}
          {enabledModes.includes("real_world") &&
            getModeButton("real_world", "Real World", "🌍")}
          {enabledModes.includes("hands_on") &&
            getModeButton("hands_on", "Hands-On", "🔬")}
        </div>
      )}
        ...
        {currentStep?.interactionType === "drag_object" && (
          <div
            style={{
              ...
              top: "20px",
              right: "20px",
              backgroundColor: "rgba(74, 77, 201, 0.9)",
              color: "#fff",
              padding: "12px 20px",
              borderRadius: "8px",
              fontSize: "14px",
