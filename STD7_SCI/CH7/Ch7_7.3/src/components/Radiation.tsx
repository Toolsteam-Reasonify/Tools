...
export type Difficulty = 'easy' | 'medium' | 'hard' | 'everyday' | 'nature' | 'technology' | 'industrial' | 'advanced';

export type QuestionType = 'mcq' | 'true-false' | 'fill-blank' | 'match' | 'ordering' | 'classify' | 'sequence' | 'true_false';

// Question interface for practice mode
...
export interface RealWorldDifficulty {
  everyday: string;
  industrial: string;
  advanced: string;
}

export interface RealWorldCategories {
  all: string;
  Kitchen: string;
  Clothing: string;
  Building: string;
...
export interface ConvectionLearnHeader {
  title: string;
  subtitle: string;
}

export interface ConvectionLearnControls {
  previous: string;
  next: string;
  play: string;
  pause: string;
...
export interface ConvectionLearnIndicators {
  practice: string;
  realWorld: string;
}
...
export interface ConvectionLearnCanvas {
  cup1: string;
  cup2: string;
  cup2Heated: string;
  hotAirRises: string;
...
export interface ConvectionLearnTranslations {
  header: ConvectionLearnHeader;
  step: string;
  controls: ConvectionLearnControls;
  indicators: ConvectionLearnIndicators;
  steps: ConvectionLearnStep[];
  canvas: ConvectionLearnCanvas;
}

export interface ConvectionPracticeHeader {
  title: string;
  subtitle: string;
}
...
export interface ConvectionPracticeQuestion {
  id: number;
  type: QuestionType;
  question: string;
  options?: string[];
...
export interface ConvectionPracticeTranslations {
  header: ConvectionPracticeHeader;
  ui: ConvectionPracticeUI;
  questions: ConvectionPracticeQuestion[];
}

export interface ConvectionRealWorldHeader {
  title: string;
  subtitle: string;
  topic: string;
}

export interface ConvectionRealWorldIntro {
  title: string;
  para1: string;
  para2: string;
  para3: string;
...
export interface ConvectionRealWorldFilter {
  title: string;
  all: string;
  showing: string;
  application: string;
...
export interface ConvectionRealWorldCategories {
  all: string;
  Nature: string;
  Home: string;
  Kitchen: string;
...
export interface ConvectionRealWorldDifficulty {
  everyday: string;
  nature: string;
  technology: string;
}

export interface ConvectionRealWorldLabels {
  howItWorks: string;
  scienceBehind: string;
  realExample: string;
  benefits: string;
...
export interface ConvectionRealWorldSummaryItem {
  title: string;
  description: string;
}

export interface ConvectionRealWorldSummary {
  title: string;
  particleMovement: ConvectionRealWorldSummaryItem;
  universalProcess: ConvectionRealWorldSummaryItem;
  planetaryImpact: ConvectionRealWorldSummaryItem;
  remember: string;
}
...
export interface ConvectionRealWorldTranslations {
  header: ConvectionRealWorldHeader;
  intro: ConvectionRealWorldIntro;
  filter: ConvectionRealWorldFilter;
  categories: ConvectionRealWorldCategories;
  difficulty: ConvectionRealWorldDifficulty;
  labels: ConvectionRealWorldLabels;
  summary: ConvectionRealWorldSummary;
...
export interface RadiationLearnHeatingWater {
...
export interface RadiationLearnTranslations {
  heatingWater: RadiationLearnHeatingWater;
}

export interface RadiationTranslations {
  learn: RadiationLearnTranslations;
}

export interface AllTranslations {
  conduction: ConductionTranslations;
  convection: ConvectionTranslations;
  radiation: RadiationTranslations;
}

export interface LanguageTranslations {
  [key: string]: AllTranslations;
}
...
// The actual translations data
export const translationsData: Translations = {    en: {
    language: {
      en: "English",
      hi: "Hindi",
      gu: "Gujarati",
  ...
  translation: {
    en: {
      conduction: {
        ...
        activity: {
          sectionTitle: "Interactive: Heat Transfer Experiment",
          safetyTitle: "⚠️ SAFETY CAUTION",
          safetyText: "This activity should be carried out under the supervision of a teacher or an adult.",
          ...
          heatFlowDirection: "Heat Flow Direction →",
          pinCard: {
            pinLabel: "Pin",
            order: "Order: {order}",
            fallen: "✓ Fallen",
          ...
          observations: {
            title: "What Did We Observe?",
            item1Title: "Pin I falls FIRST",
            item1Body: "It's closest to the heat source, so wax melts first",
            item2Title: "Pins fall in ORDER (I → II → III → IV)",
            item2Body: "Heat travels along the metal strip progressively",
            item3Title: "This proves CONDUCTION!",
            item3Body: "Heat transfers from hot end to cold end through the metal"
          },
          table: {
            title: "Table 7.1: Observation Record",
            pin: "Pin",
            order: "Order",
            status: "Status",
            reason: "Reason",
            fallen: "✓ Fallen",
            notFallen: "Not fallen",
            reasonClosest: "Closest to heat source",
            reasonProgress: "Heat reaches here progressively",
            reasonFarthest: "Farthest from heat source"
          }
        },
        testing: {
          sectionTitle: "Test Materials: Conductor or Insulator?",
          callout: "Click on materials below to test them, or drag them to the test zone!",
          zoneTitle: "Heat Transfer Testing Zone",
          dropHere: "Drop Here to Test",
          heatSource: "Heat Source",
          heatPasses: "Heat Passes!",
          heatBlocked: "Heat Blocked!",
          testResult: {
            title: "Test Result",
            material: "Material:",
            type: "Type:",
            good: "⚡ Good Conductor",
            poor: "🛡️ Poor Conductor",
            heatPasses: "✓ Heat Passes Through",
            heatStops: "✗ Heat is Blocked",
            goodDesc: "This material allows heat to flow easily, making it useful for cooking utensils.",
            poorDesc: "This material blocks heat flow, making it good for handles and insulation."
          },
          quickTest: "Quick Test:",
          tested: "Tested ({count}/{total}):"
        },
        materials: {
          steel: "Steel",
          copper: "Copper",
      ...
      convection: {
        learn: {
          header: {
            title: "Topic 7.2: Convection",
            subtitle: "Understanding Heat Transfer Through Movement"
          },
...
try {
  const injectTranslations = () => {
      Object.keys(translationsData).forEach((key) => {
          ...
          const base = translationsData[lang];
          const topics = translationsData.translation[lang];
          if (base && topics) {
               i18n.addResourceBundle(lang, 'translation', { ...base, ...topics }, true, true);
          }
      });
  };
  injectTranslations();
} catch (e) {
  console.warn('Failed to inject translations:', e);
}

export const getBaseLanguage = (lang: Language): BaseTranslations => {
  return translationsData[lang];
};

export const getTopicTranslations = (): any => {
   return translationsData.translation['en'];
};

export const getConductionTranslations = (): ConductionTranslations => {
  const lang = (i18n.language?.split('-')[0] as Language) || 'en';
  return translationsData.translation[lang]?.conduction || translationsData.translation['en'].conduction;
};

export const getConvectionTranslations = (): ConvectionTranslations => {
    const lang = (i18n.language?.split('-')[0] as Language) || 'en';
    return translationsData.translation[lang]?.convection || translationsData.translation['en'].convection;
};

export const getRadiationTranslationsNew = (): RadiationTranslations => {
    const lang = (i18n.language?.split('-')[0] as Language) || 'en';
    return translationsData.translation[lang]?.radiation || translationsData.translation['en'].radiation;
};
...
interface RadiationRealWorldTranslations {
  headerTitle: string;
  headerSubtitle: string;
  categories: {
    all: string;
...
interface RadiationRealWorldTranslationsType {
  en: RadiationRealWorldTranslations;
  hi: RadiationRealWorldTranslations;
  gu: RadiationRealWorldTranslations;
}

const realWorldTranslations: RadiationRealWorldTranslationsType = {
  en: {
...
const RadiationRealWorld: React.FC<RadiationRealWorldProps> = ({ props }) => {
  ...
  const language: LanguageCode = (props?.language ||
    ...
    "en") as LanguageCode;
  const t: RadiationRealWorldTranslations = (realWorldTranslations[language] || realWorldTranslations.en) as RadiationRealWorldTranslations;
...
interface RadiationPracticeTranslations {
  headerTitle: string;
  headerSubtitle: string;
  pointsLabel: string;
...
interface RadiationPracticeTranslationsType {
  en: RadiationPracticeTranslations;
  hi: RadiationPracticeTranslations;
  gu: RadiationPracticeTranslations;
}

const practiceTranslations: RadiationPracticeTranslationsType = {
  en: {
...
const RadiationPracticeMode: React.FC<RadiationPracticeModeProps> = ({
  ...
  const language: LanguageCode = (props?.language ||
    (contextLanguage as LanguageCode) ||
    "en") as LanguageCode;
  const t: RadiationPracticeTranslations = (practiceTranslations[language] || practiceTranslations.en) as RadiationPracticeTranslations;
  ...
  const getDifficultyLabel = (difficulty: Difficulty) => {
    // Map all difficulty types to the three main ones for display
    ...
    if (t.difficultyLabels) {
      return t.difficultyLabels[mappedDifficulty];
    }
            ...
            <div className="mb-6">
              ...
              {currentQuestion.type === "match" && (
                <div className="space-y-4">
                  {currentQuestion.options?.map((pair: string, index: number) => {
                    ...
                    return (
                      <div key={index} className="flex items-center gap-4">
                        ...
                        >
                          ...
                          {currentQuestion.options?.map((p: string, i: number) => (
