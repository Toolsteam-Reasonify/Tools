...
export default function DemonstrationMode() {
  ...
  const [selectedTypeForTool, setSelectedTypeForTool] = useState<
    'interpret' | 'percent_of_total' | 'ratio_to_percent' | 'percent_change' | null
  >(null);
    ...
    <div className={`transition-all duration-100 ease-out ${
      ...
      <div className="transition-all duration-800 ease-out">
          <div className="bg-gradient-to-br from-white to-teal-50/50 rounded-xl border border-teal-100 shadow-lg overflow-hidden">
            ...
            <div className="p-4">
              <EquationAnimation
                onSelectType={(t: 'interpret' | 'percent_of_total' | 'ratio_to_percent' | 'percent_change') => {
          ...
          <div className="bg-gradient-to-br from-white to-blue-50/50 rounded-xl border border-blue-100 shadow-lg overflow-hidden">
            ...
            <div className="p-4">
              <ComprehensivePercentageTool
                showStep={true}
                defaultSelectedType={selectedTypeForTool || 'interpret'}
                onBackToTypes={() => {
                  setShowTool(false);
                }}
                hideTypeGrid={true}
      ...
      {/* The step-by-step tool is now separate and not connected to the examples above */}
    </div>
  );
}

function EquationAnimation({ onSelectType }: { onSelectType: (t: 'interpret' | 'percent_of_total' | 'ratio_to_percent' | 'percent_change') => void }) {
  return <LinearEquationExample onSelectType={onSelectType} />;
}

function LinearEquationExample({ onSelectType }: { onSelectType: (t: 'interpret' | 'percent_of_total' | 'ratio_to_percent' | 'percent_change') => void }) {
  ...
  // Language-specific percentage examples (Topic 7.2)
  const equationExamples = {
            en: [
              { equation: "5% * ₹100 = ₹5", solution: "₹5", operation: "Interpret Percentage statement", icon: "🚗" },
              { equation: "25% × 40 = 10", solution: "10", operation: "Percentage of a total", icon: "🍼" },
              { equation: "2:1 = 2/3 × 100% ≈ 66.7%", solution: "≈66.7%", operation: "Ratio to Percentage", icon: "👥" },
              { equation: "Increase: 280→350; (70/280) × 100 = 25%", solution: "25%", operation: "Percentage change", icon: "🛒" }
            ],
    hi: [
      { equation: "₹100 * 5% = ₹5", solution: "₹5", operation: "प्रतिशत का अर्थ", icon: "🚗" },
    ...
    gu: [
      { equation: "₹100 * 5% = ₹5", solution: "₹5", operation: "ટકાનો અર્થ", icon: "🚗" },
              ...
              <div className="grid grid-cols-2 gap-4">
                {currentExamples.map((example, index) => (
                  <button
                    ...
                    key={index}
                    onClick={() => {
                      const mapping: Record<number, 'interpret' | 'percent_of_total' | 'ratio_to_percent' | 'percent_change'> = {
                        0: 'interpret',
                        1: 'percent_of_total',
                        2: 'ratio_to_percent',
                        3: 'percent_change'
