...
const FractionToPercentage: React.FC<FractionToPercentageProps> = ({ 
  ...
  const LargeCoin: React.FC<{ 
    ...
    showValue?: boolean;
    label?: string;
  }> = ({ isHighlighted, delay, size, value, showValue = false, label }) => {
    const sizeClasses = {
      small: 'w-10 h-10',
      medium: 'w-16 h-16', 
      large: 'w-20 h-20',
      ...
      <div className="flex flex-col items-center">
        ...
        {(showValue || label) && (
          <motion.div
            ...
            className="mt-2 text-xs font-bold text-gray-700 bg-white px-2 py-1 rounded shadow border text-center whitespace-nowrap"
          >
            {value || label || ''}
          </motion.div>
        )}
          ...
          <div className="flex flex-wrap gap-1 justify-center max-w-2xl">
            {coins.map((_, index) => (
              <LargeCoin
                ...
                size="small"
                showValue={true}
                value={index < highlightedCoins ? "25" : "0"}
              />
            ))}
          </div>
          <div className="text-sm text-gray-600 text-center">
