...
export default function DemoStep({ title, description, example, isActive = true, stepNumber }: Props) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl transition-all duration-700 ease-out transform ${
      isActive 
        ? 'shadow-2xl scale-105 bg-gradient-to-br from-blue-50 via-purple-50 to-blue-50 border-2 border-blue-300' 
        : 'shadow-lg bg-white border border-gray-200 hover:shadow-xl'
    }`}>
      {/* Animated Background Pattern */}
      {isActive && (
        <div className="absolute inset-0 opacity-20">
          <div className="absolute top-0 left-0 w-20 h-20 bg-gradient-to-br from-blue-400 to-purple-400 rounded-full blur-xl animate-pulse" />
          <div className="absolute bottom-0 right-0 w-16 h-16 bg-gradient-to-tl from-purple-400 to-blue-400 rounded-full blur-lg animate-pulse" style={{ animationDelay: '1s' }} />
        </div>
      )}
      ...
      <div className="relative p-6 z-10">
        ...
        <div className={`flex items-center mb-4 transition-all duration-800 ${
          ...
          {stepNumber && (
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm mr-3 transition-all duration-600 ${
              isActive 
                ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white scale-110 shadow-lg animate-bounce-gentle' 
        ...
        <div className={`relative transition-all duration-900 ease-out ${
          ...
          <div className={`p-4 rounded-lg transition-all duration-600 ${
            isActive 
              ? 'bg-white/90 backdrop-blur-sm border border-blue-200/60 shadow-inner' 
              : 'bg-gray-50/30'
          }`}>
            ...
            <div className={`p-4 rounded-lg font-mono text-lg relative overflow-hidden transition-all duration-600 ${
              ...
              <div className={`transition-all duration-700 relative text-center ${
                ...
                <div className={`inline-block px-8 py-5 rounded-2xl relative overflow-hidden transition-all duration-700 ${
                  isActive 
                    ? 'bg-white border-2 border-blue-300 shadow-xl shadow-blue-200/50 animate-math-highlight' 
                    : 'bg-white/80 border border-gray-200 shadow-sm'
                }`}>
                  {/* Clean Gradient Border Effect */}
                  {isActive && (
                    <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-blue-200 via-purple-200/50 to-blue-200 p-0.5">
                  ...
                  {/* Subtle Glow Effect */}
                  {isActive && (
                    <div className="absolute inset-0 bg-gradient-to-r from-orange-100/30 via-white/10 to-red-100/30 rounded-2xl animate-pulse opacity-50" />
                  )}
                </div>
              </div>
            </div>
