...
const Ch10: React.FC = () => {
  ...
  const handleSubmitAnswer = () => {
    ...
    if (Array.isArray(currentExercise.correctAnswer)) {
      ...
      isCorrect = studentAnswers.length === currentExercise.correctAnswer.length &&
        studentAnswers.every((ans: number, idx: number) => ans === (currentExercise.correctAnswer as number[])[idx]);
    ...
    if (isCorrect) {
      ...
      setPracticeResults((prev: PracticeResults) => {
        const newResults = {
          ...prev,
  ...
  const AlgebraicExpressionsGuide: React.FC = () => {
    ...
    const FormationSection = () => {
      ...
      const [showVariablesBlock, setShowVariablesBlock] = useState(false);
      const [showConstantsBlock, setShowConstantsBlock] = useState(false);
      ...
      React.useEffect(() => {
        if (currentSection === 0) {
          ...
          setShowVariablesBlock(false);
          setShowConstantsBlock(false);
          setShowFormationTitle(false);
          setVisibleButtons([]);
          setShowSteps(false);
          setVisibleSteps([]);

          const timer1 = setTimeout(() => setShowBuildingBlocks(true), 300);
          const timer2 = setTimeout(() => setShowVariablesBlock(true), 800);
          const timer3 = setTimeout(() => setShowConstantsBlock(true), 1200);
          const timer4 = setTimeout(() => {
            setShowFormationTitle(true);
            for (let i = 0; i < examples.length; i++) {
              setTimeout(() => {
                setVisibleButtons((prev: number[]) => [...prev, i]);
              }, 300 + (i * 400));
            }
          }, 1800);
          ...
          setTimeout(() => {
            ...
            for (let i = 0; i < current.steps.length; i++) {
              setTimeout(() => {
                setVisibleSteps((prev: number[]) => [...prev, i]);
              }, i * 300);
            }
          }, 3500);
        ...
        <div className="space-y-6">
          <div className={`bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-lg border-l-4 border-green-500 transition-all duration-500 ${
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className={`bg-white p-4 rounded-lg shadow transition-all duration-500 ${
                showVariablesBlock ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'
              }`}>
                <div className="font-bold text-green-700 mb-2">{t.variables}</div>
                <div className="text-sm text-gray-600">{t.variablesDesc}</div>
              </div>
              <div className={`bg-white p-4 rounded-lg shadow transition-all duration-500 ${
                showConstantsBlock ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'
              }`}>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-6 border-2 border-blue-300">
                  ...
                  <div className="bg-white rounded-xl p-4 shadow-md">
                    <input
                      type="text"
                      value={studentAnswer}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setStudentAnswer(e.target.value)}
                ...
                <div className="bg-green-50 p-4 rounded-xl border-2 border-green-200">
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {practiceResults.correct.length > 0 ? (
                      practiceResults.correct.map((id: string, idx: number) => (
                ...
                <div className="bg-red-50 p-4 rounded-xl border-2 border-red-200">
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {practiceResults.incorrect.length > 0 ? (
                      practiceResults.incorrect.map((id: string, idx: number) => (
                        <div key={idx} className="bg-white p-2 rounded-lg text-sm text-gray-700">
                          {practiceExercises.find(ex => ex.id === id)?.question || id}
                        </div>
                      ))
                ...
                <div className="bg-yellow-50 p-4 rounded-xl border-2 border-yellow-200">
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {practiceResults.skipped.length > 0 ? (
                      practiceResults.skipped.map((id: string, idx: number) => (
                        <div key={idx} className="bg-white p-2 rounded-lg text-sm text-gray-700">
                          {practiceExercises.find(ex => ex.id === id)?.question || id}
                        </div>
                      ))
