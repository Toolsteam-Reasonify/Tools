...
import { Lightbulb, CheckCircle, XCircle, Binary } from 'lucide-react';
...
const Ch10: React.FC = () => {
  ...
  const AlgebraicExpressionsGuide: React.FC = () => {
    ...
    // 10.3 Terms and Factors section
    const TermsSection = () => {
      ...
      React.useEffect(() => {
        setShowHeader(false); setVisibleExamples([]); setVisibleTerms({});
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                for (let j = 0; j < termExamples[i].terms.length; j++) {
                  setTimeout(() => { setVisibleTerms(prev => ({ ...prev, [i]: [...(prev[i] || []), j] })); }, j * 400);
                }
              }, 800);
            }, i * 1200);
          }
      ...
      return (
        <div className="space-y-6">
          <div className={`bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-lg border-l-4 border-purple-500 transition-all duration-500 ${showHeader ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
            <h3 className="text-xl font-bold text-purple-900 mb-3">{t.understandingTerms}</h3>
            <p className="text-gray-700">{t.understandingTermsText}</p>
          </div>
          {termExamples.map((example, exIdx) => {
            ...
            return (
              <div key={exIdx} className={`bg-white p-6 rounded-lg shadow-lg border-2 border-gray-200 transition-all duration-500 ${isExampleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
                <div className="text-2xl font-mono text-center mb-6 font-bold text-gray-800 bg-gradient-to-r from-purple-100 to-pink-100 p-4 rounded-lg">{example.expr}</div>
                <div className="grid grid-cols-2 gap-4 mb-6">
                  {example.terms.map((term, idx) => {
                    const isTermVisible = visibleTermIndices.includes(idx);
                    return (
                      <button key={idx} onClick={() => setSelectedTerm(term)} className={`p-4 rounded-lg border-2 transition-all transform hover:scale-105 ${selectedTerm === term ? 'border-purple-500 bg-purple-50 shadow-lg' : 'border-gray-300 bg-white hover:border-purple-300'} ${isTermVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
                ...
                {selectedTerm && (example as any).factors[selectedTerm as keyof typeof example.factors] && (
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-5 rounded-lg border-2 border-purple-300">
                    ...
                    <div className="flex gap-3 justify-center flex-wrap">
                      {(example as any).factors[selectedTerm]?.map((factor: string, idx: number) => (
                        <div key={idx} className="bg-white px-6 py-3 rounded-lg shadow-md border-2 border-purple-200 font-mono text-lg font-bold text-purple-700">{factor}</div>
                      ))}
                    </div>
                  </div>
                )}
    ...
    const sections = [
      { id: '10.3', title: t.sectionTerms, icon: <Binary className="w-6 h-6" />, content: <TermsSection /> }
    ];
        ...
        <div className="max-w-5xl mx-auto">
          <div className={`bg-white rounded-xl shadow-2xl p-8 mb-6 transition-all duration-300 ${animateIn ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-4'}`}>
            <div className="flex items-center gap-3 mb-6 pb-4 border-b-2 border-gray-200">
              <div className="bg-gradient-to-r from-teal-500 to-purple-500 p-3 rounded-lg text-white">{sections[0].icon}</div>
              <div>
                <div className="text-sm font-semibold text-gray-500">{t.learning} {sections[0].id}</div>
                <h2 className="text-2xl font-bold text-gray-800">{sections[0].title}</h2>
              </div>
            </div>
            <div className="min-h-[500px]">{sections[0].content}</div>
          </div>
        </div>
      </div>
    );
              ...
              <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 border-2 border-orange-200">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap justify-center gap-4">
                    ...
                    <button onClick={() => setShowOverview(true)} className="flex items-center gap-2 px-6 py-3 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-colors font-medium shadow-lg hover:scale-105">{t.overview}</button>
                  </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800">{t.practiceSummary}</h2>
                <button onClick={() => setShowOverview(false)} className="text-gray-500 hover:text-gray-700">✕</button>
              </div>
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="bg-green-50 p-4 rounded-lg border-2 border-green-200">
                  <div className="text-2xl font-bold text-green-600">{practiceResults.correct.length}</div>
                  <div className="text-sm text-gray-600">{t.overviewCorrectAnswers}</div>
                </div>
                <div className="bg-red-50 p-4 rounded-lg border-2 border-red-200">
                  <div className="text-2xl font-bold text-red-600">{practiceResults.incorrect.length}</div>
                  <div className="text-sm text-gray-600">{t.overviewIncorrectAnswers}</div>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg border-2 border-gray-200">
                  <div className="text-2xl font-bold text-gray-600">{practiceResults.skipped.length}</div>
                  <div className="text-sm text-gray-600">{t.skippedQuestions}</div>
                </div>
              </div>
              <button onClick={() => setShowOverview(false)} className="w-full px-6 py-3 bg-teal-500 text-white rounded-xl hover:bg-teal-600 transition-colors font-medium">{t.close}</button>
            </div>
          </div>
        )}
      </>
